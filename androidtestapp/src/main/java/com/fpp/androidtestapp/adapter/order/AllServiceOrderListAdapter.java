...
/**
 * @author ：fupengpeng
 * @description ：全部服务单列表Adapter
 * @data ：
 */
public class AllServiceOrderListAdapter extends BaseAdapter<OrderItem> {
     ...
     * @param data
     */
    public AllServiceOrderListAdapter(Context context, List<OrderItem> data) {
        super(context, data);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if ("1".equals(item.getState())){
            holder.tvLvItemServiceOrderStatus.setText("待接单");
        }else if ("2".equals(item.getState())){
            holder.tvLvItemServiceOrderStatus.setText("服务中");
        }else if ("8".equals(item.getState())){
            holder.tvLvItemServiceOrderStatus.setText("已完成");
        }else if ("9".equals(item.getState())){
...
//        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
        ...
        if (item.getMemberbelongsshopid().isEmpty()){
            ...
            holder.ivLvItemServiceOrderUserPic.setImageURI(Uri.parse("----------"));
